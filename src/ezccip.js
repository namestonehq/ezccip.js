	...
	toString() {
		...
		if (show) {
			...
			if (show.length) {
				// render as args
				desc += show
					.map((x) => (typeof x === "string" ? asciiize(x) : x))
					.join(",");
			}
		}
		desc += ")";
...
export class EZCCIP {
		...
		this.register(
			...
			async ([calls], context, history) => {
				...
				return [
					await Promise.all(
						calls.map((x) =>
							this.handleCall(x, context, history.enter()).catch(encode_error)
						)
					),
				];
			}
	...
	async handleRead(sender, calldata, context = {}) {
		...
		context.origin = context.origin ? getAddress(context.origin) : context.sender; // origin can be modified by the callback
		context.history = history;
		const response = await this.handleCall(calldata, context, history);
		let data;
		const expires = Math.floor(Date.now() / 1000) + (context.ttlSec ?? 60);
		switch (context.protocol ?? "tor") {
			...
			case "tor": {
				// https://github.com/namestonehq/TheOffchainResolver.sol?tab=readme-ov-file#tor-protocol
				let hash = solidityPackedKeccak256(
...
export async function processENSIP10(
	record,
	calldata,
	{multicall = true, defaultAddress = true, throwErrors} = {},
	history
) {
	try {
		...
		switch (frag.__name) {
			...
			// https://github.com/ensdomains/ens-contracts/blob/staging/contracts/resolvers/profiles/ICompositeResolver.sol
			// https://github.com/ensdomains/ens-contracts/blob/staging/contracts/resolvers/profiles/IVerifiableResolver.sol
		}
